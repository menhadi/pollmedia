...
def parse_page(text):
    ...
    for number, line in enumerate(text.splitlines(), 1):
        ...
        if (len(cells) != 6 or not re.fullmatch(r'\d{6}', cells[0])
            ...
            continue
        total, sc = [int(c) if c != '-' else None for c in cells[4:]]
        warnings = ['unverified_pdf_table', 'source_era_geography_not_joined',
                    'scheduled_tribes_not_extracted_by_this_profile']
        if '-' in cells[4:]:
...
def extract(root, package, expected, job, digest, resources_ok):
    ...
    if expected != ORIGINAL or digest(package) != expected:
    ...
    try:
        with source.open(encoding='utf-8') as stream, partial.open('w', encoding='utf-8') as target:
            for raw in stream:
                ...
                rows, failures = parse_page(page['text']); found.append(page['page'])
