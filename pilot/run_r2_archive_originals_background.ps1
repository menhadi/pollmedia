...
try {
    $log = Join-Path $root 'exports\archive-originals-r2-upload.log'
    & $settings.python (Join-Path $PSScriptRoot 'sync_archive_originals_r2.py') --workers 3 --progress-every 100 *>> $log
    if ($LASTEXITCODE -ne 0) {
        throw "Archived originals uploader exited with code $LASTEXITCODE. See $log."
    }
} finally {
