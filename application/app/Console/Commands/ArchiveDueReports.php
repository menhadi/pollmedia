...
#[Description('Save current-evidence Pilibhit drafts at quarter and year end without publishing')]
class ArchiveDueReports extends Command
{
    /**
     * Execute the console command.
     */
    public function handle(ReportController $reports, ReportArchive $archive): int
    {
        ...
        try {
            foreach ($date->month === 12 ? ['quarterly', 'annual'] : ['quarterly'] as $edition) {
                ...
                if (DB::table('report_drafts')->where('edition', $edition)->where('period', $period)
                    ->where('generated_at', '>=', $date->copy()->startOfDay()->utc())->exists()) {
                    $this->info($edition.': period-end draft already saved.');

                    continue;
                }
                $id = $archive->save($reports->draft($edition));
                $this->info($edition.': saved draft '.$id);
            }
