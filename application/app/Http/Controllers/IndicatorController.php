...
class IndicatorController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $base = DB::table('observations as v')->join('places as p', 'p.id', '=', 'v.place_id')
            ...
            ->whereIn('r.status', ['accepted', 'pending', 'needs_review', 'superseded'])->whereIn('v.status', ['reported', 'needs_review']);
        ...
        $measurements = $base->when($period !== '', fn ($q) => $q->where('v.period', $period))
            ->select('v.id', 'v.value', 'v.period', 'v.source_locator', 'p.name', 'p.slug', 'p.type', 'p.country_code',
                'i.label', 'i.unit', 'i.definition', 'i.evidence_class', 'r.id as release_id', 'r.status as release_status', 'v.status as observation_status', 'r.url', 'r.retrieved_at', 's.publisher')
