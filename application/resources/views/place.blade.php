...
</style><link rel="stylesheet" href="/css/election-dashboard.css?v={{ substr(hash_file('sha256', public_path('css/election-dashboard.css')), 0, 12) }}"></head><body class="election-ui"><header class="topbar"><a class="brand" href="/">pollmedia.</a><nav><a href="/">Places</a><a href="/india/sir">SIR explorer</a><a href="#sources">Sources</a><a href="{{ route('sources.index') }}">Data status</a></nav></header><div class="dashboard-shell {{ in_array($place->type,['pc','ac']) ? '' : 'page-without-sidebar' }}"><main class="dashboard-main">
...
<footer>Pollmedia · Local integrated pilot · Public launch and source-permission review pending.</footer></main>@if(in_array($place->type,['pc','ac']))@include('public-related-links')@endif</div><script>const input=document.getElementById('villagesearch');if(input)input.addEventListener('input',()=>{let visible=0;for(const row of document.querySelectorAll('#villagerows tr')){row.hidden=!row.textContent.toLowerCase().includes(input.value.trim().toLowerCase());if(!row.hidden)visible++}document.getElementById('villageempty').hidden=visible>0});</script></body></html>
