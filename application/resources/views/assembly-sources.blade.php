...
<section class="card scroll"><table><thead><tr><th>State</th><th>Year</th><th>Official report</th><th>Local collection</th></tr></thead><tbody>@foreach($entries as $entry)<tr><td>{{ $entry['state'] }}</td><td>{{ $entry['label'] }}</td><td><a href="{{ $entry['url'] }}">Open official edition</a>@if($entry['extraction'])<br><a href="{{ route('elections.assembly', ['edition' => $entry['extraction'], 'state' => $entry['state']]) }}">View extracted results</a>@endif</td><td>{{ $entry['collected'] }} archived files &middot; {{ str_replace('_', ' ', $entry['status']) }}</td></tr>@endforeach</tbody></table></section>
