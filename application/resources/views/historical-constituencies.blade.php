...
<p class="notice">@if(! $indexed)The searchable archive index has not been built yet. @else No indexed constituency tables match this selection. @endif You can still browse <a href="{{ route('elections.history') }}">Lok Sabha editions</a> and <a href="{{ route('elections.assembly') }}">Assembly editions</a>.</p>
@endif
...
<div><label for="year">Election year</label><select id="year" name="year"><option value="">All indexed years</option>@foreach($years as $year)<option value="{{ $year }}" @selected(($input['year'] ?? '') == $year)>{{ $year }}</option>@endforeach</select></div>
<div><label for="state">State label in source</label><select id="state" name="state"><option value="">All recorded states</option>@foreach($states as $state)<option value="{{ $state }}" @selected(($input['state'] ?? '') === $state)>{{ $state }}</option>@endforeach</select></div>
<button>Find results</button>
</form>
@if($results && $results->total())
<p>{{ number_format($results->total()) }} indexed constituency tables match. Each link opens candidate figures, the official source and any data notes.</p>
<div class="scroll"><table><thead><tr><th>Election</th><th>State as recorded</th><th>Constituency</th><th>Candidate rows</th><th>Source status</th></tr></thead><tbody>
@foreach($results as $result)<tr><td>{{ $result->edition_label }} · {{ strtoupper($result->kind) }}</td><td>{{ $result->state_label ?? 'State not identified' }}</td><td><a href="{{ route($result->kind === 'pc' ? 'elections.history' : 'elections.assembly', ['edition' => $result->edition_id, 'state' => $result->state_label, 'code' => $result->record_code]) }}">{{ $result->constituency_name }}</a></td><td>{{ number_format($result->candidate_count) }}</td><td>{{ $result->has_warning ? 'Needs review †' : 'No extraction warning' }}</td></tr>@endforeach
</tbody></table></div>
{{ $results->links() }}
@endif
@endsection
