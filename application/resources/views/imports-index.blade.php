@extends('seo-layout')
@section('content')
<h1>Official data imports</h1><p>Connect a public official URL once. Pollmedia can download its latest file or API response, extract a table and flag changes for review.</p>
...
<div class="field"><label for="url">Direct official file or API URL</label><input style="width:100%" type="url" id="url" name="url" required value="{{ old('url') }}"><small class="muted">Public HTTPS gov.in or nic.in URLs. Use the actual export endpoint, not a catalogue page. APIs requiring keys are not connected by this form.</small></div>
<div class="filters"><div><label for="format">Format</label><select id="format" name="format">@foreach(['json'=>'JSON API / file','csv'=>'CSV (UTF-8)','xlsx'=>'Excel (.xlsx)','pdf'=>'PDF tables'] as $value=>$label)<option value="{{ $value }}" @selected(old('format')===$value)>{{ $label }}</option>@endforeach</select></div><div><label for="record_key">Unique record-code column</label><input id="record_key" type="text" name="record_key" required value="{{ old('record_key') }}" placeholder="Exact column name"></div></div>
...
<p><label><input type="checkbox" name="automatic" value="1" @checked(old('automatic',1))>Check automatically each day</label></p><p class="muted">Daily checks use this computer's scheduled Pollmedia task. It must be available at the scheduled time. Limits: 20 MB, 20,000 rows, 100 PDF pages.</p><button>Save source configuration</button></form></details>
