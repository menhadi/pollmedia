...
<div class="kicker">Politics & elections / Historical results</div><h1>{{ $selected ? ($selected['constituency_name'] ?? $selected['name']) : $archiveTitle.' election results' }}</h1>@if($selected)<p>{{ $state }} &middot; {{ $data['year'] }} &middot; {{ $kind==='pc'?'Lok Sabha':'Assembly' }}</p><p><a href="{{ route('constituency.overview',['kind'=>$kind,'state'=>$state,'name'=>$selected['constituency_name']??$selected['name']]) }}">View this place across all election years &rarr;</a></p>@endif
@if($data && !$selected)<section class="panel archive-snapshot"><div class="panel-heading"><div><p class="kicker">{{ $data['year'] }} / selected election report</p><h2>{{ $state ?: $archiveTitle }} at a glance</h2></div><a href="#{{ $state ? 'state-results' : 'coverage' }}">Explore the full tables ↓</a></div><div class="data-overview"><div class="data-stat"><strong>{{ number_format($state ? $constituencies->count() : count($data['records'])) }}</strong><span>Available election results</span></div><div class="data-stat"><strong>{{ number_format($state ? $constituencies->sum(fn($r)=>count($r['candidates']??[])) : $coverage->sum('rows')) }}</strong><span>Candidate rows, including NOTA where reported</span></div><div class="data-stat"><strong>{{ $state ? $partySummary['counted'] : $states->count() }}</strong><span>{{ $state ? 'Established single-seat winners' : 'State names / codes in this edition' }}</span></div></div><p class="small">Available records only. These counts are not an official seat tally; incomplete and disputed records keep their notes.</p>
