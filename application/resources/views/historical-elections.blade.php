...
@if($data && !$selected)<section class="panel archive-snapshot"><div class="panel-heading"><div><p class="kicker">{{ $data['year'] }} / selected source edition</p><h2>{{ $state ?: $archiveTitle }} at a glance</h2></div><a href="#{{ $state ? 'state-results' : 'coverage' }}">Explore the full tables ↓</a></div><div class="data-overview"><div class="data-stat"><strong>{{ number_format($state ? $constituencies->count() : count($data['records'])) }}</strong><span>Extracted constituency tables</span></div><div class="data-stat"><strong>{{ number_format($state ? $constituencies->sum(fn($r)=>count($r['candidates']??[])) : $coverage->sum('rows')) }}</strong><span>Candidate rows, including NOTA where reported</span></div><div class="data-stat"><strong>{{ $state ? $partySummary['counted'] : $states->count() }}</strong><span>{{ $state ? 'Established single-seat winners' : 'State names / codes in this edition' }}</span></div></div><p class="small">Available extracted records only. These counts are not an official seat tally; incomplete and disputed records keep their notes.</p>
@if(!$state)<div class="table-scroll" role="region" aria-label="Edition data preview" tabindex="0"><table data-sortable><caption>Eight state groups with the most extracted constituency tables</caption><thead><tr><th scope="col">State as recorded</th><th scope="col" data-sort-type="number">Tables</th><th scope="col" data-sort-type="number">Candidate rows</th></tr></thead><tbody>@foreach($coverage->sortByDesc('tables')->take(8) as $item)<tr><th scope="row">@if($item['state'])<a href="{{ route($archiveRoute,['edition'=>$edition,'state'=>$item['state']]) }}">{{ $item['state'] }}</a>@else State not identified @endif</th><td>{{ number_format($item['tables']) }}</td><td>{{ number_format($item['rows']) }}</td></tr>@endforeach</tbody></table></div>@endif</section>@endif
...
<p class="small">{{ count($data['records']) }} extracted constituency tables in this edition. State codes are shown where a state name has not been extracted.</p>
</section>
@if($state && !$selected)
...
<p><a href="{{ route($archiveRoute, $filters + ['format' => 'csv']) }}">Download this state’s candidate results (CSV)</a></p><p class="small">Exports the available tables in this edition, with official references and current data notes. Blank cells mean not reported. Constituency totals repeat on candidate rows; do not sum them across rows.</p>
<p>{{ $stateResults->count() }} extracted constituency tables. Winners and margins appear where established for a single-seat contest. Open a candidate table for the full figures and official references.</p>
...
<p>Browse the extracted tables by the state names or codes used in this report. Counts describe the collected data, not the total seats or complete election coverage.</p>
<div class="table"><table><caption>Extracted constituency tables and candidate rows, including NOTA where reported</caption><thead><tr><th scope="col">State / union territory</th><th scope="col">Constituency tables</th><th scope="col">Extracted rows</th><th scope="col">Explore</th></tr></thead><tbody>
...
@empty<tr><td colspan="4">No constituency tables have been extracted for this edition.</td></tr>@endforelse
