...
<body><header><a class="brand" href="{{ route('home') }}">pollmedia.</a><nav><a href="{{ route('home') }}">Explore India</a><a href="#sources">Official sources</a></nav></header><main>
...
@foreach($stateResults as $record)<tr><th scope="row">{{ $record['official_pc_code'] ?? $record['code'] }} / {{ $record['constituency_name'] ?? $record['name'] }} @if($record['has_warning'])<a href="#state-note-{{ $record['code'] }}" aria-label="Data note for {{ $record['constituency_name'] ?? $record['name'] }}">†</a>@endif</th>
...
<p>Official constituency code: {{ $selected['official_pc_code'] ?? ($kind === 'ac' ? $selected['code'] : 'Not reported') }} · Seats: {{ $selected['number_of_seats'] ?? 1 }}</p>
