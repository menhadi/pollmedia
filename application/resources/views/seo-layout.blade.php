...
.dashboard-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:18px}.dashboard-grid .card{margin-bottom:0}header{flex-wrap:wrap}</style><link rel="stylesheet" href="/css/admin-shell.css"></head><body class="@auth admin-shell @endauth">
@auth @include('admin-navigation') @endauth
<div class="admin-workspace"><header class="admin-toolbar">@auth<button type="button" class="menu-toggle secondary" aria-controls="admin-sidebar" aria-expanded="true" aria-label="Toggle administration menu">&#9776;</button>@endauth<strong>Administration</strong><div class="admin-toolbar-actions"><a href="{{ route('home') }}">View website</a>@auth<a class="admin-user" href="{{ route('admin.account') }}"><span class="admin-avatar">{{ mb_substr(auth()->user()->name,0,1) }}</span><span>{{ auth()->user()->name }}<small>Your account</small></span></a><form method="post" action="{{ route('admin.logout') }}">@csrf<button class="secondary">Sign out</button></form>@endauth</div></header><main>
@if(session('status'))
...
@endif
@yield('content')
</main></div>@auth<script>
const toggle=document.querySelector('.menu-toggle');const desktop=matchMedia('(min-width:1100px)');
function setMenu(open){document.body.classList.toggle('admin-menu-open',open);toggle.setAttribute('aria-expanded',String(open));document.querySelector('#admin-sidebar').inert=!open;}
setMenu(desktop.matches);desktop.addEventListener('change',event=>setMenu(event.matches));
toggle.addEventListener('click',()=>setMenu(toggle.getAttribute('aria-expanded')!=='true'));
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!desktop.matches){setMenu(false);toggle.focus();}});
</script>@endauth</body></html>
