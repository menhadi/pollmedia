...
@auth
<a href="{{ route('imports.index') }}">Data imports</a><a href="{{ route('seo.index') }}">SEO</a><span>{{ auth()->user()->name }}</span><form method="post" action="{{ route('admin.logout') }}">@csrf<button class="secondary">Sign out</button></form>
@endauth
</div></header><main>
@if(session('status'))
