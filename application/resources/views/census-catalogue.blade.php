...
@if($admin)
<details class="card"><summary>Prepare an imported table</summary><p>Preparation creates a private preview. Accept the import baseline before publishing. Large tables can also be prepared with <code>php -d memory_limit=512M artisan census:prepare</code>.</p>
...
@endif
<form class="card filters" method="get"><div><label for="edition">Census year and source table</label><select name="edition" id="edition">@foreach($editions as $option)<option value="{{ $option->id }}" @selected($edition?->id === $option->id)>{{ $option->year }} · {{ $option->name }} @if($admin) · {{ $option->status }} · #{{ $option->id }} @endif</option>@endforeach</select></div><button @disabled($editions->isEmpty())>Load table</button></form>
@if($edition)
...
@if($admin)
<section class="card"><h2>Publication review</h2><p>{{ $edition->row_count }} records · {{ $edition->flag_count }} records with validation notes · {{ $edition->status }}</p><a href="{{ route('imports.run', $edition->import_run_id) }}">Review and accept source baseline</a>
