...
class PublicSearchTest extends TestCase
{
    ...
    public function test_search_combines_typed_places_historical_results_and_villages(): void
    {
        ...
        $this->getJson('/search?q=Pilibhit')->assertOk()->assertJsonStructure(['suggestions' => [['label', 'type', 'url']]])->assertJsonFragment(['type' => 'PC · Uttar Pradesh']);
        $this->getJson('/search?q=P')->assertOk()->assertExactJson(['suggestions' => []]);
        $this->get('/search?q=Pilibhit')->assertOk()->assertSee('Parliament (PC)')->assertSee('Assembly (AC)')->assertSee('Pilibhit Test Village')->assertSee('/india/village/123456-pilibhit-test-village', false)->assertDontSee('PC · 2024');
