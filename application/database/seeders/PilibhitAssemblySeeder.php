...
class PilibhitAssemblySeeder extends Seeder
{
    /**
    ...
    {
        $fixtures = json_decode(file_get_contents(database_path('fixtures/pilibhit-assembly.json')), true, 512, JSON_THROW_ON_ERROR);
        foreach ($fixtures as $data) {
            app(ElectionResults::class)->validate($data);
        }
        DB::transaction(function () use ($fixtures): void {
            foreach ($fixtures as $data) {
                $place = DB::table('places')->where('slug', 'ac-'.$data['slug'])->value('id');
                $key = 'eci-up-ac-'.$data['code'].'-'.$data['year'];
                ...
                $contest = DB::table('election_contests')->insertGetId(['place_id' => $place, 'source_release_id' => $release, 'year' => $data['year'], 'election_type' => 'vidhan_sabha_general', 'source_locator' => $data['source_locator'], 'electors' => $data['electors'], 'votes_polled' => $data['votes_polled'], 'valid_candidate_votes' => $data['valid_candidate_votes'], 'active' => ! $existing]);
